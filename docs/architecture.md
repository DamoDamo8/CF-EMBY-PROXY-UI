...
- 主记录信号是通过节点、安全和路由前置检查的 `GET /Users/<UserId>/Items/<ItemId>`。请求必须包含 `EnableImageTypes` 且含 `Primary`、`ImageTypeLimit=1`、`Fields` 且含 `ProviderIds` 与 `ExternalUrls`，并携带 `Authorization`、`X-Emby-Token` 或 `X-MediaBrowser-Token` 之一。Worker 只克隆成功的 JSON 响应，以 256 KiB 上限解析 `Id`、`Name`、`Type`、`SeriesName`、`OriginalTitle`、`ProductionYear` 与 `ImageTags.Primary`；路径与正文 ItemId 必须一致。裸 `/Items/<id>`、图片子资源、缺参数、缺授权、非 2xx、非 JSON、无稳定设备/身份和超限正文均不写入。
- 成功的 `POST /Items/<ItemId>/PlaybackInfo?IsPlayback=true` 是主记录缺失时的保底。intent 只在本次成功响应上标记播放资格，不保存 ItemId；PlaybackInfo 路径与本次 JSON 自包含地解析媒体，不再与用户 Item GET 或 `Playing`/`Progress` 的 ItemId 做跨请求关联。同节点同身份在 2 分钟有界上下文中已存在用户 Item 详情时，迟到 PlaybackInfo 不覆盖主来源。
- `POST /Sessions/Playing`、`POST /Sessions/Playing/Progress` 与 `POST /Sessions/Playing/Stopped` 不再更新服务器记录观看时间或媒体指针；它们继续执行既有上游代理、Progress 节流和可选双向进度镜像。用户 Item 与 PlaybackInfo 观察写入都使用请求进入时的 `startTime`，通过 `ctx.waitUntil()` 调用兼容的最近观看 D1 batch；解析或写入失败不得改变正文、请求头、上游转发或 Emby 响应。
- `server_last_watch` 与 `server_record_snapshots` 继续按 `node_name` 条件更新，较旧或跨 isolate 乱序观察不得回拨时间。已发布的 v9 lifecycle 列与 migration 保留用于 schema/历史数据兼容，但新观察不再依赖阶段或会话指纹。读取仍只向管理台下发已有媒体字段、同源 `posterUrl` 和固定 `posterSearch`；图片 Tag、用户 ID、Provider ID、Token、Cookie、上游地址和完整 JSON 不得出现在快照或日志。手动刷新仍按用户 Item 详情 → PlaybackInfo 补齐当前 ID-only 指针，浏览器海报兜底保持同源 Emby → TMDB → 豆瓣。
