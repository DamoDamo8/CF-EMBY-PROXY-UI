...
- Hills 未在 `Playing`/`Progress` 正文中提供 ItemId 时，Worker 只将成功的 `POST /Items/<ItemId>/PlaybackInfo?IsPlayback=true` 与成功 JSON `GET /Users/<UserId>/Items/<ItemId>` 详情响应在同节点、同会话或 DeviceId 下做 2 分钟 isolate 内关联；两种证据的 ItemId 必须完全一致才可补齐名称、类型、剧集名和 Primary ImageTag。上下文 Map 有界且仅保存最小媒体字段和脱敏身份摘要，不保存用户 ID、Token、Cookie、会话原文、上游地址或完整详情正文；没有稳定身份、跨节点/设备、非 JSON/失败详情、普通详情浏览和 `/Items/<id>/Images/Primary` 图片请求均不能创建或补全观看媒体指针。详情晚于生命周期时不追溯改写已保存的观看时间。
...
- `server_last_watch` 与 `server_record_snapshots` 在同一 D1 batch 中条件更新：只有时间更晚且满足阶段/指纹门禁的事件才能改变观看表，第二条语句仅跟随本次已接纳写入更新媒体指针。重复、乱序和跨 isolate 并发事件不得回拨时间或覆盖海报。STOP 缺媒体时只复用同 isolate 可确认的同会话媒体；无法确认时清空媒体指针。v8 表下 Playing 与 STOP 走旧写入，Progress 持久化关闭且 SQL 不引用 v9 列；执行“初始化 DB”补齐 v9 并失效 readiness 后自动启用完整指纹去重。
...
- 根 `migrations/` 是 D1 schema 版本与结构契约的正式真相源。v9 升级链依次为新库基础表 `0001_d1_fresh_baseline.sql`、历史库兼容表 `0002_d1_historical_compatibility.sql`、索引收口 `0003_d1_schema_v5_indexes.sql`、节点最后观看表 `0004_server_watch_stats.sql`、服务器资源/媒体快照表 `0005_server_record_snapshots.sql`、海报解析缓存表 `0006_server_record_poster_cache.sql` 和播放生命周期字段 `0007_server_watch_lifecycle.sql`。SQLite migration 不猜测任意旧表缺失列；历史库由受控兼容初始化确认并补齐登记列，新库可直接满足全部版本契约。
...
- `initLogsDb` 是唯一受控 schema mutation 入口。它在任何 DDL/DML 前获取 Time Travel bookmark；获取失败必须零写入终止。之后统一执行全库兼容检查、缺表创建、已知非键列补齐、命名索引校验/重建、退役索引删除、异常 `proxy_logs_fts` 重建和最终状态复检。结构完整后才创建或校验 `d1_migrations`，并以幂等写入采纳缺失的 0001–0007 基线，最终必须返回 `schemaVersion: 9`、`migrationReady: true`、`adoptedMigrations` 与初始化前 `recoveryBookmark`。`getD1SchemaStatus`、`initD1Schema` 与 `initLogsFts` 仅保留 API 兼容，不得登记 migration。
...
- `runtimeCompatibilityReady` 只表示当前结构满足 Worker v9 兼容读取；`migrationReady` 还要求 `d1_migrations` 表有效且七个要求 migration 均已记录。只有二者条件满足时 `schemaVersion` 才返回 `9`。缺失 migration 只能由已登录用户显式触发的 `initLogsDb` 在 bookmark 和结构复检通过后采纳；运行时兜底不得登记版本。
