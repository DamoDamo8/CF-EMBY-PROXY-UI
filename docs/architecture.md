...
- 影视资源版本聚合使用配置中的 `mediaAggregationNodes` 组成无状态节点池。请求任一池内节点的 `PlaybackInfo` 时，Worker 先读取主服 `ProviderIds`，再按“节点 `mediaAggregationEmbyUsername` / `mediaAggregationEmbyPassword` > 全局 `mediaAggregationEmbyUsername` / `mediaAggregationEmbyPassword`”解析有效凭据，登录其他池节点，按 TMDB 或 IMDB 精确匹配并追加备服 `MediaSources`。节点或全局账号存在即视为有效凭据，密码允许为空并按空字符串登录；没有节点账号且没有全局账号的节点不能加入聚合池。聚合与服务器记录的固定账号令牌只在 isolate 内缓存 10 分钟，不写入 KV/D1；服务器记录令牌缓存键显式覆盖节点、节点 revision、目标和凭据摘要，节点写入会清除关联条目。最多并发读取 8 个备服，任一备服失败不影响主服响应。
...
- 服务器探测只在管理动作中执行：活动线路优先，每条线路先请求 `/System/Ping`，只有 Ping 成功才选定为在线目标；HTTP 错误、超时或网络失败继续沿节点线路顺序回退，所有线路仅返回 401/403 时状态为 `unauthorized`。配置服务器记录专用账号或继承节点固定账号时，选定目标后先调用 `/Users/AuthenticateByName`；登录请求和其后的详情请求不得携带继承的代理 Token 或 Cookie，只能使用新令牌。认证失败时跳过三项 `/Items` 统计，且不得回退使用节点代理认证头；未配置账号的旧节点继续使用原有自定义认证头。认证成功后，三项 `/Items` 与可选 `/System/Info` 并行；System Info 只补充版本与 ServerId，不能参与状态判断。单请求 8 秒超时、最多 4 节点并发。探测结果按 `nodeName` 在 isolate 内缓存 60 秒，缓存值不含上游地址或认证头，节点写入立即失效本 isolate 条目。
- 服务器记录登录令牌只存在于 isolate 内：最多 32 项、最长 10 分钟，缓存键必须同时覆盖节点名称及派生 revision、目标线路和账号密码摘要，且不得向浏览器、KV 或 D1 暴露令牌。相同键的并发登录以 single-flight 合并，与影视资源聚合登录使用独立缓存 namespace。显式刷新按节点和 revision 合并探测；当已有 60 秒探测快照且上次结果为可重试的离线失败时，后续强制刷新复用该快照并按 1 秒起、60 秒封顶的指数退避返回 `probe.source: "backoff"` 与 `retryAt`。节点写入或失效同时清除探测快照、服务器记录令牌与退避状态。
...
- 管理台只展示“初始化 DB”。该动作通过 `initLogsDb` 统一执行全库运行时兼容检查、缺表创建、已知非键列补齐、命名索引校验/重建、退役索引删除、异常 `proxy_logs_fts` 重建和最终状态复检；`getD1SchemaStatus`、`initD1Schema` 与 `initLogsFts` 继续保留为 API 兼容入口，不再形成第二套 UI 初始化流程，也不得伪造 migration 记录。
- 自动修复白名单只包括当前契约登记的表、可追加的已知非键列、命名普通索引、退役普通索引和可再生 FTS 表。初始化必须先对全部已存在的同名业务表完成只读关键约束预检；任一主键、`proxy_logs.id` 整数主键语义或 `dns_ip_pool_items.ip` 唯一键不匹配时，在任何 `ALTER TABLE`、建表、索引或 FTS DDL 之前 fail-closed。未知表/索引不删除，键列缺失不自动重建业务表，最终状态返回 `runtimeCompatibilityReady: false` 并保留原数据。`PRAGMA table_info` / `index_list` / `index_xinfo` 失败返回 `D1_SCHEMA_INSPECTION_FAILED`，不得按空结构继续。
