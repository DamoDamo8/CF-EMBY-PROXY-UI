...
本文负责 Worker 壳、路由职责、运行时绑定、资源代理和缓存语义。管理台内部结构见 [管理台契约](admin-console.md)，发布资产和 URL 推导见 [构建与发布](release.md)。服务器记录海报的已实现浏览器直连边界见 [服务器记录海报重构契约](poster-contract.md)。
...
- 只有通过节点、安全和路由前置检查的 `POST /Sessions/Playing`、`POST /Sessions/Playing/Progress` 与 `POST /Sessions/Playing/Stopped` 进入记录生命周期；GET、HEAD、其他方法和 Ping 不写入。调用位于代理请求状态构建之后、媒体聚合改写与 Progress 节流提前返回之前，使用请求进入时的 `startTime` 并通过 `ctx.waitUntil()` 异步写 D1；解析和写入失败不得改变正文、请求头、上游转发或 Emby 响应。
- 生命周期解析器只复用代理准备阶段已经缓冲且不超过 256 KiB 的正文，接受 `application/json`、`application/*+json`、`text/json` 和 Hills 使用的 `text/plain` JSON object；`text/plain` 普通文本、JSON 数组和无效 JSON 均拒绝。未知长度、超限或流式正文不额外读取，解析失败仍可使用 query 的 `ItemId`、`SessionId`、`PlaySessionId` 与 `DeviceId`。内部只记录 `parseMode` 与脱敏错误原因，不记录正文、原始会话标识或凭据；转发始终保留原正文与原请求头。
- Hills 未在 `Playing`/`Progress` 正文中提供 ItemId 时，Worker 只将成功的 `POST /Items/<ItemId>/PlaybackInfo?IsPlayback=true` 与成功 JSON 详情响应（优先 `GET /Users/<UserId>/Items/<ItemId>`，兼容裸 `GET /Items/<ItemId>`）在同节点、同会话或 DeviceId 下做 2 分钟 isolate 内关联；两种证据的 ItemId 必须完全一致才可补齐标题、原始标题、年份、类型和剧集名。上下文 Map 有界且仅保存这些最小媒体字段和脱敏身份摘要，不保存图片 Tag、Provider ID、用户 ID、Token、Cookie、会话原文、上游地址或完整详情正文；没有稳定身份、跨节点/设备、非 JSON/失败详情，以及 `/Items/<id>/Images/*`、`PlaybackInfo` 等子资源请求均不能创建或补全观看媒体指针。详情晚于生命周期时不追溯改写已保存的观看时间。
- 播放写入侧只使用代理流程已经缓冲的请求或响应元数据。单集以所属系列的名称、原始名称和年份生成海报检索身份；电影与剧集分别规范为 `movie`、`tv`。Worker 不为海报主动读取 Emby、不保存图片 Tag 或供应商 ID，也不调度供应商搜索、图片预取或缓存写入。
- `Playing` 在存在 `ItemId` 时立即更新最后观看时间、滚动到期基准和媒体指针；缺少 Playing 时，首次可识别的 `Progress` 作为兜底，同会话后续 Progress 只刷新内存观察时间；`Stopped` 始终尝试推进最终时间。同 isolate 的独立有界 `ServerRecordWatchSessions` Map 保存指纹、强弱等级、ItemId、媒体、写入状态和观察/终止时间，不复用进度转发 Map；STOP 保留 10 分钟 terminal tombstone，只拦截同一身份且同一媒体指纹的迟到 Progress，同一身份切换 ItemId 必须作为新指纹继续兜底。非终止会话无活动 30 分钟清理，容量沿用播放进度会话上限。节点改名、删除、导入替换或 revision 失效会清理相关条目。
- D1 v9 `server_last_watch` 以 `node_name` 为唯一主键，在原观看时间外保存 64 位 FNV-1a 生成的 16 位小写十六进制 `playback_session_fingerprint`、`playback_session_strength` 与 `playback_event_phase`；不保存 SessionId、PlaySessionId、DeviceId 或身份原文。强指纹由节点、SessionId/PlaySessionId 和 ItemId 派生；弱指纹由节点、DeviceId 和 ItemId 派生，缺少 DeviceId 时使用同格式的脱敏身份摘要。弱 Playing 即使与旧指纹相同也表示明确重播；强 Progress 只在指纹变化时兜底；弱 Progress 在指纹变化、同指纹 STOP 已超过 10 分钟，或同指纹未 STOP 且旧记录超过 12 小时时接纳，其余跳过。
- `server_last_watch` 与 `server_record_snapshots` 在同一 D1 batch 中条件更新：只有时间更晚且满足阶段/指纹门禁的事件才能改变观看表，第二条语句仅跟随本次已接纳写入更新媒体指针。重复、乱序和跨 isolate 并发事件不得回拨时间或覆盖最近媒体。STOP 缺媒体时只复用同 isolate 可确认的同会话媒体；无法确认时清空媒体指针。读取时只要指针有 `itemId`，且两个时间均可解析、媒体时间不晚于最后观看时间并最多落后 5 分钟，就向管理台下发已有媒体字段和固定 `posterSearch`；名称为空时前端显示 ItemId。v8 表下 Playing 与 STOP 走旧写入，Progress 持久化关闭且 SQL 不引用 v9 列；执行“初始化 DB”补齐当前 schema 并失效 readiness 后自动启用完整指纹去重。
- 结构化访问日志只增加 `watchPhase`、`watchDecision`、`watchParseMode` 与 `watchSessionStrength`。生命周期 D1 仍由 `ctx.waitUntil()` 执行；访问日志仅延迟入队到该任务完成，并在入队前刷新最终决策，不得让代理响应等待 D1。决策值限定为 `scheduled`、`deduped`、`fallback_progress`、`fallback_playback_context`、`skipped_no_item`、`skipped_unbuffered`、`late_progress_after_stop`、`schema_v8_fallback`、`d1_unavailable` 等脱敏状态，不得包含媒体正文、原始会话 ID、Token、Cookie、密码、上游地址或 TMDB Key。
- 仅已启用的服务器记录会写入，关闭记录不会删除已有时间。节点改名必须在同一节点 mutation 的补偿链中迁移观看生命周期字段和快照：名称冲突时最后观看时间及其会话字段、与之同时间的媒体指针取最新值，媒体统计按最新 `stats_checked_at` 独立合并；节点删除必须删除两张表的同名行，回滚则恢复变更前行，避免旧名称重建后继承历史数据。
...
- 快照的 `watch.posterSearch` 固定返回 `{ itemId, mediaType, title, originalTitle, year, watchedAt }`；`mediaType` 仅为 `movie` 或 `tv`，缺失文本为空字符串，缺失年份为 `null`。响应不包含 `posterUrl`、图片 Tag 或供应商 ID。旧海报路径不再注册，统一进入普通 `404`，且不会触发 Emby 或供应商请求。
- 首张海报卡片进入可视区后，前端通过已登录、`no-store` 的 `getPosterBrowserConfig` 一次取得浏览器专用配置；管理台 KV 值逐项优先，Cloudflare Dashboard binding 兜底。设置 bootstrap 只返回 Token 状态、来源和非敏感的管理台 Origin。TMDB 按中文标题、原始标题依次搜索前三项候选，只接受类型、名称和年份唯一精确匹配；失败后调用配置 origin 的豆瓣 resolve/poster 接口。搜索与图片请求均为 8 秒超时、`redirect: "error"`，图片限制 5 MiB，并要求允许的 MIME 与 JPEG、PNG、WebP、AVIF 或 GIF 文件签名一致。
- 前端用 `IntersectionObserver` 和最多 16 路原生队列按需解析；卡片移除、离开页面或退出登录时取消请求并释放 Blob URL，退出登录同时清空内存凭据。错误只记录供应商和固定错误码。浏览器 `localStorage` 键 `server-record-poster:v1` 使用 SHA-256 搜索身份、256 项 LRU、成功 7 天和失败 30 分钟；首次清理其他同前缀旧键。普通加载复用成功或失败缓存，手动刷新只绕过失败缓存且不预取不可见卡片。
...
- `HOST`
- `LEGACY_HOST`
- `TMDB_BROWSER_TOKEN`（管理台未保存 `tmdbBrowserToken` 时使用的 TMDB token）
- `DOUBAN_BROWSER_ORIGIN`（管理台未保存 `doubanBrowserOrigin` 时使用的独立豆瓣服务 HTTPS origin）
- `DOUBAN_BROWSER_TOKEN`（管理台未保存 `doubanBrowserToken` 时使用的豆瓣 Bearer token）
...
`wrangler.toml` 当前声明 `ENI_KV`、`DB` 和每小时 Cron，并启用了 `enable_request_signal`。服务器最后观看记录与过期预警去重直接复用 `DB`，不需要 Durable Object。`cfAccountId`、`cfZoneId`、`cfApiToken`、`tgBotToken` 和 `tgChatId` 是保存到 KV 的后台设置项，不是必需的 Worker 环境变量；退役的 `tmdbApiKey` 不再属于运行配置。

## D1 schema

- 根 `migrations/` 是 D1 schema 版本与结构契约的正式真相源。v11 升级链依次为 `0001_d1_fresh_baseline.sql`、`0002_d1_historical_compatibility.sql`、`0003_d1_schema_v5_indexes.sql`、`0004_server_watch_stats.sql`、`0005_server_record_snapshots.sql`、历史海报缓存 `0006_server_record_poster_cache.sql`、播放生命周期 `0007_server_watch_lifecycle.sql`、历史豆瓣字段 `0008_server_record_poster_douban.sql`，以及为快照补充原始标题/年份并删除退役缓存表的 `0009_drop_server_record_poster_cache.sql`。SQLite migration 不猜测任意旧表缺失列；历史库由受控兼容初始化确认并补齐登记列，新库可直接满足全部版本契约。
...
- `initLogsDb` 是唯一受控 schema mutation 入口。它在任何 DDL/DML 前获取 Time Travel bookmark；获取失败必须零写入终止。之后统一执行全库兼容检查、缺表创建、已知非键列补齐、命名索引校验/重建、退役索引删除、异常 `proxy_logs_fts` 重建和最终状态复检。v11 初始化幂等补齐 `last_item_original_title`、`last_item_year` 并删除 `server_record_poster_cache`；这些步骤全部成功后才登记 `0009`。最终必须返回 `schemaVersion: 11`、`migrationReady: true`、`adoptedMigrations` 与初始化前 `recoveryBookmark`。`getD1SchemaStatus`、`initD1Schema` 与 `initLogsFts` 仅保留 API 兼容，不得登记 migration。
...
- 手动、scheduled 和底层 `applyD1TidyPlan` 都必须在首个清理步骤前通过 `runtimeCompatibilityReady` 门禁。手动 D1 tidy 可幂等删除意外残留的 `server_record_poster_cache`，但不迁移、不备份、不登记 migration；scheduled 不读取或删除该表，也不能采纳 migration 基线。D1 tidy 之后才执行保留期删除、统计/FTS 维护和 `PRAGMA optimize`。
- `getD1SchemaStatus` 直接复检 `sqlite_master`、运行时表的完整必需列、主键、`dns_ip_pool_items.ip` 唯一键、命名索引所属表与完整键列顺序，以及 `d1_migrations`；`server_record_snapshots` 必须存在 `last_item_original_title`、`last_item_year`，且 `server_record_poster_cache` 必须不存在。FTS readiness 还必须确认 `proxy_logs_fts` 是绑定 `content=proxy_logs`、`content_rowid=id` 的 FTS5 虚拟表，并且插入触发器按契约向 rowid 与五个检索列写入对应的新日志字段。状态分别报告 `runtimeCompatibilityVersion`、`runtimeCompatibilityReady`、`appliedMigrations`、`latestRequiredMigration`、`missingMigrations`、`migrationReady`、`schemaVersion`、表/列/索引/约束/FTS readiness 与 `issues`。`proxy_logs_fts` 不进入基础 migration，FTS 创建或重建失败不得报告 `ftsReady: true`。
- `runtimeCompatibilityReady` 只表示当前结构满足 Worker v11 兼容读取；`migrationReady` 还要求 `d1_migrations` 表有效且九个要求 migration 均已记录。只有二者条件满足时 `schemaVersion` 才返回 `11`。缺失 migration 只能由已登录用户显式触发的 `initLogsDb` 在 bookmark 和结构复检通过后采纳；运行时兜底不得登记版本。
