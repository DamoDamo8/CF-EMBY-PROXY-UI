...
- 上次观看由节点代理被动解析合格的用户 Item GET 并即时更新；成功的 `IsPlayback=true` PlaybackInfo 只作自包含保底，`Playing`、`Progress` 与 `Stopped` 不写服务器记录观看时间。`Name` 原样作为卡片标题，剧集以 `SeriesName` 生成 `posterSearch`。海报卡片进入可视区后先读取已认证同源 Emby 海报，失败后浏览器按 [海报重构契约](poster-contract.md) 直连供应商并显示 Blob URL；手动刷新可修复 ID-only 指针，但仍不预取不可见卡片。
