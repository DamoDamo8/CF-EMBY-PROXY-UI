...
- 本地/预发必须验证不同 `nodeName` 独立写入、GET/HEAD 控制请求不写、Playing 后不退出也能显示时间/媒体、首次强 Progress 兜底且后续高频 Progress 零重复写、弱 Playing 重播、STOP 后 10 分钟同指纹门禁、未 STOP 的 12 小时门禁，以及重复、乱序和并发事件不回拨。播放写入侧优先复用同节点 Item 详情响应的 `Name/Type/SeriesName/OriginalTitle/ProductionYear/ImageTags.Primary`，PlaybackInfo 顶层字段次之，覆盖缺失元数据与标题原样展示；单集供应商搜索仍使用系列身份。快照只返回同源 `posterUrl`，不得返回图片 Tag、供应商 ID、上游 URL 或凭据；播放写入任务不得额外读取 Emby、预热图片或搜索标题，只有手动刷新可以为当前 ID-only 指针补读详情。
